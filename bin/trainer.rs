...
const SB1: usize = 600;
const SB2: usize = 200;
const MAX_WEIGHT: f32 = 127. / QH as f32;
...
impl Orchestrator {
    ...
    fn run(&self) -> Result<(), Failure> {
        let mut trainer = ValueTrainerBuilder::default()
            ...
            .save_format(&[
                save("ftb").round().quantise::<i16>(QT),
                save("ftw").round().quantise::<i16>(QT),
                save("l12b").round().quantise::<i32>(QH * 127),
                save("l23b").round().quantise::<i32>(QH * 127),
                save("l3ob").round().quantise::<i32>(QH * 127),
                save("l12w").round().quantise::<i8>(QH as _).transpose(),
                save("l23w").round().quantise::<i8>(QH as _).transpose(),
                save("l3ow").round().quantise::<i8>(QH as _).transpose(),
            ])
            .use_win_rate_model()
            .loss_fn(|output, pt| {
