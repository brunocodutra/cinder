...
impl Iterator for SortedRatedMovesIter<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if cursor >= self.inner.unsorted.cast::<usize>() {
            ...
            const SHIFT: u32 = 32 - <Rating as Num>::Repr::BITS;
            const { assert!(SHIFT >= Moves::CAPACITY.next_power_of_two().trailing_zeros()) }

            let mut best = i32::MIN;
            for (i, rating) in ratings.iter().enumerate() {
                best = best.max((rating.cast::<i32>() << SHIFT) | i.cast::<i32>());
            }

            let idx = cursor + (best.cast::<usize>() & ones::<usize>(SHIFT));
...
mod tests {
    ...
    fn sorted_iterates_through_moves_by_highest_rating(
        mut ms: RatedMoves,
        #[strategy(-99..=99i16)] base: i16,
    ) {
        for (i, r) in ms.ratings.iter_mut().enumerate() {
            *r = Rating::new(base) + i.cast::<i16>();
        }

        let mut rms = Vec::from_iter(ms.iter().copied());
        rms.reverse();

        assert_eq!(Vec::from_iter(ms.sorted()), rms);
    }
