...
use std::{cmp::Reverse, convert::Infallible, iter::Map, num::NonZeroU16};
...
#[cfg_attr(test, filter(#self.is_promotion() || #self.encode().slice(12..14) == zero()))]
#[repr(transparent)]
...
#[cfg_attr(test, derive(test_strategy::Arbitrary))]
#[cfg_attr(test, arbitrary(bound(R)))]
#[repr(transparent)]
pub struct Moves<R: Copy + Eq = ()>(
    #[deref(forward)]
    #[deref_mut(forward)]
    #[cfg_attr(test, strategy(vec(any::<(Move, R)>(), 0..=10usize)
        .prop_map(StaticSeq::from_iter)))]
    StaticSeq<(Move, R), 255>,
);

impl<R: Copy + Eq> FromIterator<(Move, R)> for Moves<R> {
    #[inline(always)]
    fn from_iter<I: IntoIterator<Item = (Move, R)>>(iter: I) -> Self {
...
impl<R: Default + Copy + Eq> FromIterator<Move> for Moves<R> {
    #[inline(always)]
    fn from_iter<I: IntoIterator<Item = Move>>(iter: I) -> Self {
        iter.into_iter().map(|m| (m, R::default())).collect()
    }
}

impl IntoIterator for Moves {
    type Item = Move;
    type IntoIter = Map<StaticSeqIter<(Move, ()), 255>, fn((Move, ())) -> Move>;

    #[inline(always)]
    #[cfg_attr(feature = "no_panic", no_panic::no_panic)]
    fn into_iter(self) -> Self::IntoIter {
        self.0.into_iter().map(|(m, ())| m)
    }
}

impl<'a> IntoIterator for &'a Moves {
    type Item = &'a Move;
    type IntoIter = Map<Iter<'a, (Move, ())>, fn(&'a (Move, ())) -> &'a Move>;
...
impl<'a> IntoIterator for &'a mut Moves {
    ...
    type IntoIter = Map<IterMut<'a, (Move, ())>, fn(&'a mut (Move, ())) -> &'a mut Move>;

    #[inline(always)]
    #[cfg_attr(feature = "no_panic", no_panic::no_panic)]
    fn into_iter(self) -> Self::IntoIter {
        self.0.iter_mut().map(|(m, ())| m)
    }
}
...
impl FromIterator<RatedMove> for RatedMoves {
    #[inline(always)]
...
impl FromIterator<Move> for RatedMoves {
    #[inline(always)]
    fn from_iter<I: IntoIterator<Item = Move>>(iter: I) -> Self {
        RatedMoves {
            entries: iter.into_iter().collect(),
...
impl IntoIterator for RatedMoves {
    type Item = RatedMove;
    ...
    #[cfg_attr(feature = "no_panic", no_panic::no_panic)]
    fn into_iter(self) -> Self::IntoIter {
        self.entries.0.into_iter()
    }
}

impl<'a> IntoIterator for &'a RatedMoves {
...
impl<'a> IntoIterator for &'a mut RatedMoves {
...
impl<'a> SortedRatedMovesIter<'a> {
    #[inline(always)]
    #[cfg_attr(feature = "no_panic", no_panic::no_panic)]
    fn new(moves: &'a mut RatedMoves) -> Self {
        SortedRatedMovesIter { moves, cursor: 0 }
    }
}
...
impl MoveCollector for Moves {
    ...
    fn collect_attacks(
        ...
        for wt in targets & !theirs {
            ...
            self.0.extend_from_simd(moves, attacks.to_array()[wt]);
        }
    ...
    fn collect_pawn_promotions(
        ...
        let turn = pos.turn();
        let wt = targets & !pos.occupied();
        let pawns = pos.by_piece(Piece::new(Role::Pawn, turn)) & !pos.pins().mask();
        ...
        if mask != 0 {
            ...
            let moves = PUSHES[turn].as_ref::<u16x32>();
            self.0.extend_from_simd(*moves, expanded_mask);
        }

        let theirs = pos.by_color(!turn);
        let indices = pos.roles()[turn].matching(Some(Role::Pawn));
        let attacks = pos.pins().attacks().to_simd() & u16x64::splat(*indices);
        for wt in targets & theirs & attacks.simd_ne(zeroed()) {
            for idx in indices & attacks.to_array()[wt] {
    ...
    fn collect_pawn_pushes(
        ...
        if mask != 0 {
            let moves = SINGLE[turn].as_ref::<u16x32>();
            self.0.extend_from_simd(*moves, mask);
        }

        let double = match turn {
            Color::White => pawns & (empty >> 8) & (wt >> 16),
            Color::Black => pawns & (empty << 8) & (wt << 16),
        };
        ...
        if mask != 0 {
            let moves = DOUBLE[turn].as_ref::<u16x16>();
            self.0.extend_from_simd(*moves, mask);
        }
