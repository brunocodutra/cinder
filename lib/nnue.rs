...
pub struct Nnue {
    ...
    nn: [Input<Residual<Hidden<Hidden<Hidden<Output>>>>>; Phase::LEN],
}
...
impl Nnue {
    const fn new() -> Self {
        ...
        while phase < Phase::LEN {
            ...
            let mut weight = [[0i8; L1::LEN]; Ln::LEN];
            cursor += unsafe { copy_bytes(&mut weight, &bytes[cursor..]) };
            let mut blocks = [[0i8; 4]; L1::LEN * Ln::LEN / 4];
            arrange_in_blocks(&weight, &mut blocks);
            interleave(&blocks, &mut nn.weight.0, 2 * Ln::LEN);
            phase += 1;
        }
        ...
        while phase < Phase::LEN {
            ...
            let mut weight = [[0f32; 2 * Ln::LEN]; Ln::LEN];
            cursor += unsafe { copy_bytes(&mut weight, &bytes[cursor..]) };
    ...
    #[inline(always)]
    pub fn nn(phase: Phase) -> &'static Input<Residual<Hidden<Hidden<Hidden<Output>>>>> {
