...
type ResidualBlock<T> = Residual<Hidden<Hidden<T>>>;

/// An Efficiently Updatable Neural Network.
#[derive(Debug, Zeroable)]
pub struct Nnue {
    transformer: Transformer,
    nn: [Input<ResidualBlock<ResidualBlock<Output>>>; Phase::LEN],
}
...
impl Nnue {
    const fn new() -> Self {
        ...
        while phase < Phase::LEN {
            ...
            let mut weight = [[0i8; L1::LEN]; L2::LEN];
            cursor += unsafe { copy_bytes(&mut weight, &bytes[cursor..]) };
            let mut blocks = [[0i8; 4]; L1::LEN * L2::LEN / 4];
        ...
        while phase < Phase::LEN {
            ...
            let mut weight = [[0f32; Ln::LEN]; Ln::LEN];
            cursor += unsafe { copy_bytes(&mut weight, &bytes[cursor..]) };
        ...
        let mut phase = 0;
        while phase < Phase::LEN {
            let nn = &mut nnue.nn[phase].next.next.next.next.next;
            let mut weight = [[0f32; Ln::LEN]; Ln::LEN];
            cursor += unsafe { copy_bytes(&mut weight, &bytes[cursor..]) };
            arrange_in_blocks(&weight, &mut nn.weight.0);
            phase += 1;
        ...
        let mut phase = 0;
        while phase < Phase::LEN {
            let nn = &mut nnue.nn[phase].next.next.next.next.next.next;
            let mut weight = [[0f32; Ln::LEN]; Ln::LEN];
            cursor += unsafe { copy_bytes(&mut weight, &bytes[cursor..]) };
            arrange_in_blocks(&weight, &mut nn.weight.0);
            phase += 1;
        ...
        let mut phase = 0;
        while phase < Phase::LEN {
            let nn = &mut nnue.nn[phase].next.next.next.next.next.next.next;
            cursor += unsafe { copy_bytes(&mut nn.weight.0, &bytes[cursor..]) };
            phase += 1;
        }
    ...
    #[inline(always)]
    pub fn nn(phase: Phase) -> &'static Input<ResidualBlock<ResidualBlock<Output>>> {
