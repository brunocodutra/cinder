...
impl Evaluator {
    ...
    fn refresh(&mut self, side: Color) {
        ...
        self.accumulator[side][self.ply] = cache.accumulator;
    ...
    fn update(&mut self, side: Color, ply: Ply) {
        ...
        if self.moves[ply - 1].is_none() {
            *dst = *src;
        } else {
            let new = &self.positions[ply];
            let old = &self.positions[ply - 1];
            accumulate(side, new.king(side), old, new, |sub, add| {
...
#[inline(always)]
#[cfg_attr(feature = "no_panic", no_panic::no_panic)]
fn accumulate<F>(side: Color, ksq: Square, old: &Placement, new: &Placement, mut accumulate: F)
where
    F: FnMut([Option<Feature>; 2], [Option<Feature>; 2]),
{
    let diff = Bitboard::from(old.pieces().simd_ne(new.pieces()));
    let mut to_sub = Squares::new(diff & old.occupied());
    let mut to_add = Squares::new(diff & new.occupied());
