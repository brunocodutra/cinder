...
#[repr(transparent)]
struct Param<const C: Bytes<64>> {
    #[cfg(feature = "spsa")]
    #[cfg_attr(test, strategy(vec(-1e3f32..=1e3f32, C.as_slice::<f32>().len())
        .prop_map(|vs| Bytes::new(&vs))))]
    values: Bytes<64>,
}

impl<const C: Bytes<64>> Param<C> {
    #[inline(always)]
    ...
    pub fn perturb<I: IntoIterator<Item = f32>>(&self, perturbations: I) -> (Self, Self) {
        ...
        for (i, c) in C.as_slice::<f32>().iter().enumerate() {
            let delta = c.abs() * perturbations.next().unwrap();
            left.values.as_mut_slice::<f32>()[i] += delta;
            right.values.as_mut_slice::<f32>()[i] -= delta;
        }
    ...
    pub fn update<I: IntoIterator<Item = f32>>(&mut self, corrections: I) {
        ...
        for (i, c) in C.as_slice::<f32>().iter().enumerate() {
            ...
            self.values.as_mut_slice::<f32>()[i] += delta;
        }
    }
}

impl<const C: Bytes<64>> Default for Param<C> {
    #[inline(always)]
    fn default() -> Self {
        Self::new()
    }
...
#[cfg(feature = "spsa")]
impl<const C: Bytes<64>> From<Param<C>> for Vec<f32> {
    fn from(param: Param<C>) -> Self {
        param.get().to_vec()
    }
}

#[cfg(feature = "spsa")]
impl<const C: Bytes<64>> From<Vec<f32>> for Param<C> {
    fn from(values: Vec<f32>) -> Self {
        Self {
            values: Bytes::new(&values),
        }
    }
}
...
macro_rules! params {
    ($($name: ident: $value: expr,)*) => {
        ...
        pub struct Params {
            $(#[cfg_attr(feature = "spsa", serde(default))] $name: Param<{ Bytes::new::<f32>(&$value) }>,)*
        }
...
mod tests {
    ...
    #[proptest]
    fn perturbing_updates_params(p: Params, #[any(size_range(Params::LEN).lift())] d: Vec<f32>) {
