...
macro_rules! define_constant {
    ($name:ident, $n:expr, $raw:literal) => {
        ...
        #[repr(transparent)]
        pub struct $name<const V: [i32; $n]> {}

        impl<const V: [i32; $n]> $name<V> {
            pub const fn new() -> Self {
                Self {}
            }
        }

        impl<const V: [i32; $n]> Param for $name<V> {
            const LEN: usize = 0;
            type Value = [i32; $n];
        ...
        impl<const V: [i32; $n]> From<$name<V>> for [i32; $n] {
            fn from(_: $name<V>) -> Self {
                V
            }
        }

        impl<const V: [i32; $n]> TryFrom<[i32; $n]> for $name<V> {
...
define_constant!(Constant1, 1, "[i32; 1]");
define_constant!(Constant2, 2, "[i32; 2]");
define_constant!(Constant3, 3, "[i32; 3]");
define_constant!(Constant4, 4, "[i32; 4]");
define_constant!(Constant5, 5, "[i32; 5]");
define_constant!(Constant6, 6, "[i32; 6]");
define_constant!(Constant7, 7, "[i32; 7]");
define_constant!(Constant8, 8, "[i32; 8]");
define_constant!(Constant9, 9, "[i32; 9]");
...
macro_rules! define_variable {
    ($name:ident, $n:expr, $raw:literal) => {
        ...
        impl<const V: [i32; $n]> Default for $name<V> {
            fn default() -> Self {
                Self::new()
            }
        }

        impl<const V: [i32; $n]> From<$name<V>> for [f64; $n] {
            fn from(variable: $name<V>) -> Self {
                variable.values
            }
        }

        impl<const V: [i32; $n]> TryFrom<[f64; $n]> for $name<V> {
            type Error = ValueOutOfRange;
...
mod tests {
    ...
    fn constant_has_an_equivalent_if_value(p: MockConstant) {
        assert_eq!(MockConstant::try_from(<[i32; 3]>::from(p)), Ok(p));
    }
    ...
    fn converting_variable_succeeds_if_value_in_range(
        #[strategy([
            MockVariable::min()[0] as f64..=MockVariable::max()[0] as f64,
            MockVariable::min()[1] as f64..=MockVariable::max()[1] as f64,
            MockVariable::min()[2] as f64..=MockVariable::max()[2] as f64])]
        v: [f64; 3],
    ) {
        assert_ne!(MockVariable::try_from(v), Err(ValueOutOfRange));
    }

    #[proptest]
    fn converting_variable_from_fails_if_value_too_large(
        #[strategy([
            MockVariable::max()[0] as f64 + 1.0..,
            MockVariable::max()[1] as f64 + 1.0..,
            MockVariable::max()[2] as f64 + 1.0..])]
        v: [f64; 3],
    ) {
        assert_eq!(MockVariable::try_from(v), Err(ValueOutOfRange));
    }

    #[proptest]
    fn converting_variable_from_fails_if_value_too_small(
        #[strategy([
            ..=MockVariable::min()[0] as f64 - 1.0,
            ..=MockVariable::min()[1] as f64 - 1.0,
            ..=MockVariable::min()[2] as f64 - 1.0])]
        v: [f64; 3],
    ) {
        assert_eq!(MockVariable::try_from(v), Err(ValueOutOfRange));
    }
