...
struct Stack {
    pv: Pv,
    pos: Evaluator,
    attention: Option<NonNull<Nodes>>,
    continuation: [Option<NonNull<PieceToHistory>>; Ply::LEN],
    correction: [Option<NonNull<Correction<1>>>; Ply::LEN],
    killers: [Killers; Ply::LEN],
    values: [Value; Ply::LEN],
}
...
impl<'a> Searcher<'a> {
    ...
    fn evaluate(&mut self) -> Value {
        ...
        let piece_counts = f32x8::from_array(array::from_fn(|i| match i.convert() {
            Some(r) => pos.by_role(r).count() as f32,
            None => 0.0,
        }));
    ...
    fn ab<const IS_PV: bool, const N: usize>(
        ...
        if depth < 1.0 {
            Ok(self.quiesce::<IS_PV>(bounds, is_cut)?.truncate())
        } else if self.stack.pos.ply() >= N as i32 {
            Ok(self.pvs::<IS_PV, 0>(depth, bounds, is_cut)?.truncate())
        } else {
            self.pvs::<IS_PV, N>(depth, bounds, is_cut)
        }
    }

    /// The zero-window alpha-beta search.
    #[inline(always)]
    fn nw(&mut self, depth: f32, beta: Score, is_cut: bool) -> Result<Pv<0>, Interrupted> {
        if depth >= 1.0 {
            self.pvs::<false, 0>(depth, beta - 1..beta, is_cut)
        } else {
            self.qnw(beta, is_cut)
        }
    }

    /// The zero-window quiescent search.
    #[inline(always)]
    fn qnw(&mut self, beta: Score, is_cut: bool) -> Result<Pv<0>, Interrupted> {
        self.quiesce::<false>(beta - 1..beta, is_cut)
    }

    /// The quiescent search.
    #[inline(always)]
    fn quiesce<const IS_PV: bool>(
        &mut self,
        bounds: Range<Score>,
        is_cut: bool,
    ) -> Result<Pv<0>, Interrupted> {
        self.stack.attention.update(1);
        let ply = self.stack.pos.ply();
        if self.ctrl.check(zeroed(), ply, &self.stack.pv) == Abort {
        ...
        for m in sorted_moves {
            ...
            let mut next = self.next(Some(m));
            let pv = match -next.qnw(-alpha, !is_cut)? {
                pv if pv <= alpha || pv >= beta => pv,
                _ => -next.ab::<IS_PV, _>(zeroed(), -beta..-alpha, false)?,
            };
    ...
    fn pvs<const IS_PV: bool, const N: usize>(
        ...
        let (lower, upper) = match self.shared.syzygy.wdl_after_zeroing(&self.stack.pos) {
            ...
            Some(wdl) => {
                ...
                if lower >= upper || upper <= alpha || lower >= beta {
                    let best = transposition.and_then(|t| t.best);
                    let depth = Params::tb_depth_bonus(0).add(depth).saturate();
                    let tpos = Transposition::new(score, depth, best, IS_PV || was_pv);
                    self.shared.tt.store(self.stack.pos.zobrists().hash, tpos);
        ...
        if alpha >= beta || upper <= alpha || lower >= beta || ply >= Ply::MAX {
            ...
            if !beta.is_losing() && !stand_pat.is_winning() {
                ...
                if is_cut && (ours & !(pawns ^ kings)).any() {
        ...
        if let Some(t) = transposition {
            ...
            if is_fh && was_cut && !was_quiet && depth_bounds.contains(&depth) {
                for m in moves.sorted() {
                    ...
                    let mut next = self.next(Some(m));
                    let pv = match -next.qnw(-pc_beta + 1, false)? {
        ...
        for (index, m) in moves.sorted().skip(1).enumerate() {
            ...
            if pv > alpha && lmr > *Params::lmr_threshold(0) {
                pv = -next.nw(next_depth, -alpha, !is_cut)?.truncate();
            }
    ...
    fn root(
        ...
        if self.ctrl.check(depth.saturate(), zeroed(), &self.stack.pv) != Continue {
            return Err(Interrupted);
        }
        ...
        let value = self.stack.value(0).assume();
        if !is_check && head.is_quiet() && !score.range(zeroed()).contains(&value) {
            self.update_correction(depth, score);
        }
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        fn bootstrap(search: &mut Search<'_, '_>, cx: &mut Context<'_>) -> Poll<Option<Info>> {
            ...
            search.execution = Some(executor.execute(move |idx| {
                ...
                for info in Searcher::new(local_ctrl, shared, local, stack).aw(&moves) {
                    if idx == 0 {
...
mod tests {
    ...
    fn ab_aborts_if_time_is_up(
        ...
        assert_eq!(
            searcher.ab::<true, 1>(d.cast(), b, is_cut),
            Err(Interrupted)
        );
    }
    ...
    fn aw_extends_time_to_find_some_pv(
        ...
        let mut searcher = Searcher::new(ctrl, &e.shared, &mut e.local[0], stack);
        let last = searcher.aw(&searcher.stack.pos.moves()).last();
