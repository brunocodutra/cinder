...
impl Permute for u8x64 {
    ...
    fn permute(self, indices: Self) -> Self {
        unsafe {
            ...
            let y0 = _mm256_blendv_epi8(x0.permute(j0).into(), x1.permute(j0).into(), m0.into());
            let y1 = _mm256_blendv_epi8(x0.permute(j1).into(), x1.permute(j1).into(), m1.into());

            transmute::<[u8x32; 2], Self>([y0.into(), y1.into()])
        }
    }
...
impl Permute for u8x32 {
    ...
    fn permute(self, indices: Self) -> Self {
        unsafe {
            ...
            transmute::<[u8x16; 2], Self>([y0.into(), y1.into()])
        }
    }
