...
pub trait TableDescriptor {
    ...
    const METRIC: Metric;
    /// File extension, e.g., `rtbw` or `rtbz`.
...
fn nibble_to_piece(p: u8) -> Option<Piece> {
    let role: Role = (p & !8).checked_sub(1)?.convert()?;
    let color = Color::from(p & 8 != 0);
...
fn parse_pieces(
    ...
    for p in file.read(ptr..ptr + count)? {
        pieces.push(unwrap!(nibble_to_piece(match side {
            Color::White => *p & 0xf,
            Color::Black => *p >> 4,
        })));
    }
...
impl GroupData {
    pub fn new(pieces: Pieces, order: [u8; 2], file: usize) -> io::Result<GroupData> {
        ensure!(pieces.len() >= 2);
...
impl PairsData {
    pub fn parse<T: TableDescriptor>(
        file: &RandomAccessFile,
        mut ptr: usize,
        groups: GroupData,
    ) -> io::Result<(PairsData, usize)> {
        let flags = Flag::new(file.read_u8_at(ptr)?);

        if flags.contains(&Flag::SINGLE_VALUE) {
            let single_value = if T::METRIC == Metric::Wdl {
                file.read_u8_at(ptr + 1)?
            } else {
                0
            };
        ...
        // Read header.
        let header = file.read(ptr..ptr + 10)?;

        let tb_size = groups.factors[groups.lens.len()];
        let block_size = unwrap!(1u32.checked_shl(header[1] as _));
        ensure!(block_size <= MAX_BLOCK_SIZE as u32);
        let span = unwrap!(1u32.checked_shl(header[2] as _));
        let sparse_index_size = tb_size.div_ceil(span as u64) as u32;
        let padding = header[3];
        let blocks_num = LE::read_u32(&header[4..]);
        let block_length_size = unwrap!(blocks_num.checked_add(padding as _));
        ...
        for i in (0..h - 1).rev() {
            ...
            let b = unwrap!(base[i + 1].checked_add(file.read_u16_le_at(ptr)? as _));
            base[i] = unwrap!(b.checked_sub(file.read_u16_le_at(ptr + 2)? as _)) / 2;
            ensure!(base[i] * 2 >= base[i + 1]);
        }

        for (i, base) in base.iter_mut().enumerate() {
            *base = unwrap!(base.checked_shl(64 - (min_symlen as u32 + i as u32)));
        }

        // Initialize symbols.
        ptr += 10 + h * 2;
        let sym = file.read_u16_le_at(ptr)?;
        ptr += 2;
        let btree = ptr;
        let mut symbols = vec![Symbol::new(); sym as _];
        let mut visited = vec![false; symbols.len()];
        for s in 0..sym {
            read_symbols(file, btree, &mut symbols, &mut visited, s, 16)?;
        }
...
fn read_symbols(
    ...
    if *unwrap!(visited.get(sym as usize)) {
        return Ok(());
    }

    let mut symbol = Symbol::new();
    let offset = btree + 3 * sym as usize;
    let data = file.read(offset..offset + symbol.lr.len())?;
    ...
    if symbol.right() == 0xfff {
        ...
        let depth = unwrap!(depth.checked_sub(1));

        read_symbols(file, btree, symbols, visited, symbol.left(), depth)?;
        read_symbols(file, btree, symbols, visited, symbol.right(), depth)?;

        let l = symbols[symbol.left() as usize].len;
        let r = symbols[symbol.right() as usize].len;
        symbol.len = unwrap!(unwrap!(l.checked_add(r)).checked_add(1));
    }
...
impl BlockLengthBuffer {
    ...
    fn fill_buffer(
        ...
        let data = file.read(offset..offset + self.buffer.len())?;
...
impl<T: TableDescriptor> fmt::Debug for Table<T> {
...
impl<T: TableDescriptor> Table<T> {
    ...
    pub fn new(path: PathBuf, material: NormalizedMaterial) -> io::Result<Table<T>> {
        ...
        // Read layout flags.
        let layout = Layout::new(raf.read_u8_at(4)?);
        ...
        let files: ArrayVec<_, 4> = io::Result::from_iter((0..num_files).map(|file| {
            ...
            let sides: ArrayVec<_, 2> =
                Result::from_iter(Color::iter().take(num_sides).map(|side| {
                    let pieces = parse_pieces(&raf, ptr, material.count(), side)?;
                    let key = Material::from_iter(pieces.clone());
                    ensure!(key.normalize() == material);
                    GroupData::new(pieces, order[side as usize], file)
                }))?;
        ...
        // Setup pairs.
        let mut files: ArrayVec<_, 4> = io::Result::from_iter(files.into_iter().map(|file| {
            Ok(FileData {
                sides: io::Result::from_iter(file.into_iter().map(|side| {
        ...
        for file in &mut files {
            for side in &mut file.sides {
                ...
                ptr = unwrap!(ptr.checked_add(side.blocks_num as usize * side.block_size as usize));
            }
        }
    ...
    fn decompress_pairs(&self, d: &PairsData, idx: u64) -> io::Result<u16> {
        ...
        let offset = unwrap!(d.data.checked_add(block as u64 * d.block_size as u64)) as usize;
        ...
        loop {
            ...
            while buf < *unwrap!(d.base.get(len)) {
                len += 1;
            }

            sym = ((buf - d.base[len]) >> (64 - len - d.min_symlen as usize)) as u16;
            let offset = d.lowest_sym as usize + 2 * len;
            sym += self.raf.read_u16_le_at(offset)?;

            if lit_idx < i64::from(unwrap!(d.symbols.get(sym as usize)).len) + 1 {
                break;
            }

            lit_idx -= i64::from(unwrap!(d.symbols.get(sym as usize)).len) + 1;
        ...
        let mut symbol = unwrap!(d.symbols.get(sym as usize));
        loop {
            if symbol.len == 0 {
                return Ok(symbol.left());
            }

            let left_symbol = unwrap!(d.symbols.get(symbol.left() as usize));
            if lit_idx < i64::from(left_symbol.len) + 1 {
                symbol = left_symbol;
            } else {
                lit_idx -= i64::from(left_symbol.len) + 1;
                symbol = unwrap!(d.symbols.get(symbol.right() as usize));
            }
        }
    }
    ...
    fn encode(&self, pos: &Position) -> io::Result<Option<(&PairsData, u64)>> {
        let key = Material::from_iter(pos.iter().map(|(p, _)| p));
        ...
        for piece in side.groups.pieces.iter().skip(lead_pawns_count) {
            ...
            let sq = unwrap!(candidates.into_iter().next());
    ...
    fn read_sparse_index(&self, d: &PairsData, idx: u64) -> io::Result<(u32, i64)> {
        let main_idx = idx / d.span as u64;
        ensure!(main_idx <= u32::MAX as u64);
    ...
    fn read_block_lengths(
        ...
        if lit_idx < 0 {
            // Backward scan.
            while lit_idx < 0 {
                block = unwrap!(block.checked_sub(1));
            ...
            loop {
                ...
                block = unwrap!(block.checked_add(1));
            }
        }
...
impl WdlTable {
    pub fn probe(&self, pos: &Position) -> io::Result<Wdl> {
        let (side, idx) = self.encode(pos)?.assume();
...
impl DtzTable {
    pub fn probe(&self, pos: &Position, wdl: Wdl) -> io::Result<Option<u16>> {
        ...
        let res = match &side.dtz_map {
            Some(map) => map.read(&self.raf, wdl, res)?,
            None => res,
        };
